...
export default function LoginPage() {
    ...
    return (
        <div className="font-display bg-background-light dark:bg-background-dark min-h-[calc(100vh-80px)] flex items-center justify-center p-4 antialiased text-nordic dark:text-gray-100 relative overflow-hidden">
            ...
            <main className="w-full max-w-md z-10">
                <div className="text-center mb-10">
                    ...
                    <h1 className="text-3xl font-bold tracking-tight text-nordic dark:text-white mb-2">{t('Login', 'welcome')}</h1>
                    <p className="text-nordic/60 dark:text-gray-400">{t('Login', 'unlock')}</p>
                </div>
                <div className="bg-surface dark:bg-[#152e2a] rounded-2xl shadow-soft p-8 sm:p-10 border border-white/50 dark:border-primary/20 backdrop-blur-sm">
                    <div className="space-y-4">
                        <button onClick={handleGoogleSignIn} className="group w-full flex items-center justify-center gap-3 bg-white dark:bg-[#1a3833] border border-gray-100 dark:border-primary/30 rounded-lg p-3.5 text-nordic dark:text-white font-medium transition-all duration-300 hover:shadow-soft-hover hover:-translate-y-0.5 relative overflow-hidden">
                            ...
                            <span className="relative z-10">{t('Login', 'continueGoogle')}</span>
                        </button>
                        <button onClick={handleGithubSignIn} className="group w-full flex items-center justify-center gap-3 bg-white dark:bg-[#1a3833] border border-gray-100 dark:border-primary/30 rounded-lg p-3.5 text-nordic dark:text-white font-medium transition-all duration-300 hover:shadow-soft-hover hover:-translate-y-0.5 relative overflow-hidden">
                            ...
                            <span className="relative z-10">{t('Login', 'continueGithub')}</span>
                        </button>
                    </div>
                    <p className="mt-8 text-center text-sm text-nordic/70 dark:text-gray-400">
                        {t('Login', 'noAccount')}
                        <a className="font-semibold text-primary hover:text-primary-dark transition-colors ml-1" href="#">{t('Login', 'signUp')}</a>
                    </p>
                </div>
                <div className="mt-8 text-center">
                    <nav className="flex justify-center gap-6 text-xs text-nordic/50 dark:text-gray-500">
                        <a className="hover:text-nordic dark:hover:text-gray-300 transition-colors" href="#">{t('Login', 'privacy')}</a>
                        <a className="hover:text-nordic dark:hover:text-gray-300 transition-colors" href="#">{t('Login', 'terms')}</a>
                        <a className="hover:text-nordic dark:hover:text-gray-300 transition-colors" href="#">{t('Login', 'help')}</a>
                    </nav>
                </div>
            </main>
        </div>
